...
double infoExchangeTime = 0.0;
double globalCommTime = 0.0;
double idleTime = 0.0;
...
void Solve()
{
  ...
  double tComm, tGlobal, tComp = 0.0;
  ...
  while ((count < max_iter) && (r1 > precision_goal))
  {
    ...
    tGlobal = MPI_Wtime() - tGlobal;
    globalCommTime += tGlobal;
...
int main(int argc, char **argv)
{
  ...
  cleanUpTime = MPI_Wtime();
  Write_Grid();

  Clean_Up();
  cleanUpTime = MPI_Wtime() - cleanUpTime;

  stop_timer();
  print_timer();

  if (proc_rank == 0) {
      printf("Global Comm Time: %lf\n", globalCommTime);
      printf("Info Exchange Time: %lf\n", infoExchangeTime);
      printf("Computation Time: %lf\n", computationTime);
      printf("SetupTime : %lf\n", setupTime);
      printf("CleanupTime: %lf\n", cleanUpTime);
      idleTime = wtime - setupTime - cleanUpTime - globalCommTime - infoExchangeTime - computationTime;
      printf("Idle Time: %lf\n", idleTime);
  }
