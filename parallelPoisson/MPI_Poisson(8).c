...
/* variables for measuring exchange  borders */
...
void Exchange_Borders()
{
    ...
    MPI_Sendrecv(
            &phi[1][dim[Y_DIR]-2], 1, border_type[Y_DIR], 
            process_bottom, 0, &phi[1][0], 1, 
            border_type[Y_DIR], process_top, 
            0, grid_comm, &status
    );
    /* all traffic in direction "top */
    MPI_Sendrecv(
            &phi[1][1], 1, border_type[Y_DIR], 
            process_top, 0, &phi[1][dim[Y_DIR]-1], 1, 
            border_type[Y_DIR], process_bottom, 
            0, grid_comm, &status
    );

    /* all traffic in direction "left" */
    MPI_Sendrecv(
            &phi[1][1], 1, border_type[X_DIR], 
            process_left, 1, &phi[dim[X_DIR]-1][1], 1, 
            border_type[X_DIR], process_right, 
            1, grid_comm, &status
    );
    ...
    MPI_Sendrecv(
            &phi[dim[X_DIR]-2][1], 1, border_type[X_DIR], 
            process_right, 1, &phi[0][1], 1, 
            border_type[X_DIR], process_left, 
            1, grid_comm, &status
    );
    if (process_rank == 0) {
        current_time = MPI_Wtime() - current_time;
        communication_time[communication_count] = current_time;
...
void Setup_Grid()
{
    ...
    if ((source[0] = malloc(dim[Y_DIR] * dim[X_DIR] * sizeof(**source))) == NULL)
        Debug("Setup_Subgrid : malloc(*source) failed", 1);
    if ((communication_time = malloc(max_iter * 2 * sizeof(double))) == NULL) {
        Debug("Failed setting up communication time", 0);
    }
...
double Do_Step(int parity)
{
    ...
    double omega = 1.95;
    double diff;
    int combined_offset = offset[X_DIR] + offset[Y_DIR];
    /* calculate interior of grid */
    for (x = 1; x < dim[X_DIR] - 1; x++) {
        int start = ((combined_offset + x + 1) % 2 == parity ) ? 1: 2;
        for (y = start; y < dim[Y_DIR] - 1; y = y + 2) {
            if (source[x][y] != 1) {
                old_phi = phi[x][y];
...
void Write_Exchange_Time()
{
    int x;
    FILE *f;

    char filename[40];
    sprintf(filename, "exchange_time%i.dat", process_rank);

    if ((f = fopen(filename, "w")) == NULL)
        Debug("Write_Exchange Time : fopen failed", 1);

    Debug("Write_Exchange Time", 0);

    for (x = 0; x < communication_count; x++) {
        fprintf(f, "%i: %.7f \n", x, communication_time[x]);
    }
...
void calculate_communication_time()
{
    Write_Exchange_Time();
    double sum = 0.0;
    int idx = 0;
    for (idx = 0; idx < communication_count; idx ++) {
        sum += communication_time[idx];
    }

    double average_time = sum/communication_count;

    // we're measuring always in the right direction
    communication_size = sizeof(double) * (dim[Y_DIR] - 2);

    printf("Data Length: %ld bytes; Avg Time: %.7f seconds\n", communication_size, average_time);
}
