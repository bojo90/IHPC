...
// function for monitoring error as a function of iteration
double *error_monitor;
...
void Setup_Grid()
{
    ...
    if ((source[0] = malloc(dim[Y_DIR] * dim[X_DIR] * sizeof(**source))) == NULL)
        Debug("Setup_Subgrid : malloc(*source) failed", 1);
    if ((error_monitor = malloc(max_iter * sizeof(*error_monitor))) == NULL)
        Debug("Setup Error Monitor: malloc(*error_monitor)", 1);

    error_monitor[0] = global_delta;
...
void Solve()
{
    ...
    while (global_delta > precision_goal && count < max_iter)
    {
        ...
        if (process_rank == 0) {
            error_monitor[count] = global_delta;
        }
    }
...
void Write_Error_Monitor()
{
    FILE *f;
    char filename[40];
    sprintf(filename, "error%i.csv", 0);

    if ((f = fopen(filename, "w")) == NULL)
        Debug("Write_Error: failed", 0);

    for (int idx = 0; idx < number_iterations; idx++) {
        fprintf(f, "%i,%.5f\n",idx, error_monitor[idx]);
    }
...
int main(int argc, char **argv)
{
    ...
    if (process_rank == 0) Write_Error_Monitor();
