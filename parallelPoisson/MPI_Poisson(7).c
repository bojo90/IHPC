...
/* global exchange time*/
double global_exchange = 0.0;
...
void Solve()
{
    ...
    double time1, time2, actual_time;
    ...
    while (global_delta > precision_goal && count < max_iter)
    {
        ...
        time2 = MPI_Wtime();
        Exchange_Borders();
        actual_time += MPI_Wtime() - time2;

        delta = max(delta1, delta2);
        count++;
        MPI_Allreduce(&delta, &global_delta, 1, MPI_DOUBLE, MPI_MAX, grid_comm);
        MPI_Allreduce(&actual_time, &global_exchange, 1, MPI_DOUBLE, MPI_SUM, grid_comm); 
    }

    printf("Number of iterations : %i - Processor Rank: %d\n", count, process_rank);
    if (process_rank == 0) {
        printf("Time spent in Exchange Border: %.6f\n", global_exchange); 
    }
}
